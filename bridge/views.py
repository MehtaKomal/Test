...
def add_lecture(request):
    ...
    from datetime import datetime, timedelta
    date_from = datetime.now() - timedelta(days=1)
    add_lect = AddLecture.objects.filter(
        expert_id_id=expert_login.id, update_timestamp__gte=date_from)
    ...
    for each_lectr in add_lect:
        student_list = []
        student_req_list = []
        st = StudentLectureAttended.objects.filter(lecture_id_id=each_lectr.id).filter(expert_id_id=expert_login.id)
        if st:
            for each_st in st:
                student_list.append(each_st.student_id.name)
            student_attending[each_lectr.id] = student_list
        st_req = ActivityRequest.objects.filter(activity_id=each_lectr.id).filter(expert_id=expert_login.id)
        if st_req:
...
def accept_request(request, student_id='', activity_id='', expert_id=''):
    StudentLectureAttended.objects.update_or_create(student_id_id=student_id,expert_id_id=expert_id,lecture_id_id=activity_id)
    ActivityRequest.objects.filter(student_id=student_id).filter(activity_id=activity_id).delete()
...
def send_activity_request(request, activity_id='', student_id='', expert_id=''):
    activity = ActivityRequest.objects.update_or_create(student_id=student_id,expert_id=expert_id,activity_id=activity_id)
    return HttpResponseRedirect('/coreHike/show_expert_profile_by_student/'+expert_id)
...
def recent_activity_student(request, student_id=''):
    ...
    context['news'] = news
    context['news_count'] = news.count()
    return render(request, 'recent_activity_student.html', context)


def show_expert_profile_by_student(request, expert_id=''):
    try:
        if request.user.is_authenticated:
            student_login = check_student_login(request.user.email)
            show_expert = Expert.objects.get(id=expert_id)
